...
function CreateShipmentContent() {
  ...
  useEffect(() => {
    // If returning from auth payment flow, ensure we're on step 2
    if (sessionStorage.getItem('intendedNextAction') === 'payment') {
  ...
  return (
    <div className="flex-1 overflow-auto bg-[#f8f9fa] flex flex-col pt-24 md:pt-28">
      <div className="p-6 md:p-8 max-w-6xl mx-auto w-full flex-1 pb-32 xl:pb-8">
        {/* Progress Indicator */}
        <motion.div 
        ...
        {step === 1 ? (
          ...
          >
            ...
            {/* Sticky mobile CTA */}
            <div className="xl:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
              <button
                onClick={handleGetEstimate}
                disabled={isLoadingEstimate}
                className="w-full relative rounded-xl bg-primary p-4 text-[15px] font-black text-white hover:bg-primary-600 transition-all flex justify-center items-center gap-2"
              >
                {isLoadingEstimate ? (
                  <>
